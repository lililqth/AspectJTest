...
public class Editor {
	...
	private static String parentpath = "C:"; // 文件路径
	...
	public static void createConsole() {
		...
		// 添加Console
		Text consoleText = new Text(centerComposite, SWT.BORDER | SWT.WRAP | SWT.V_SCROLL | SWT.MULTI);
	...
	public static void createEditTab() {
		tabFolder = new TabFolder(centerComposite, SWT.H_SCROLL | SWT.V_SCROLL | SWT.CANCEL | SWT.MULTI);
	...
	public static void createFileTree() throws IOException {
		...
		fileTabFolder = new TabFolder(shell, SWT.H_SCROLL | SWT.V_SCROLL | SWT.CANCEL | SWT.MULTI);
		// 在这个tab中显示函数列表
			...
			public void handleEvent(Event event) {
				...
				Listener textListener = new Listener() {
					public void handleEvent(final Event e) {
						switch (e.type) {
						case SWT.FocusOut:
							File renamed = renameFile((File) item.getData(), text.getText());
							if (renamed != null) {
						...
						case SWT.Traverse:
							...
							case SWT.TRAVERSE_RETURN:
								renamed = renameFile((File) item.getData(), text.getText());
								if (renamed != null) {
									item.setText(text.getText());
									item.setData(renamed);
								}
				...
				try {
					...
					for (int i = 0; i < variatelog.size(); i++) {
						...
						if (vcp.oldValue != null) {
							item.setText(new String[] { tmpMethodStrings[tmpMethodStrings.length - 1], vcp.oldValue,
									vcp.newValue });
						} else {
							item.setText(new String[] { tmpMethodStrings[tmpMethodStrings.length - 1], "尚未初始化",
									vcp.newValue });
						}
